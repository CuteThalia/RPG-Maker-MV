...
//    I just ask you leave a special thanks to Quasi somewhere.
//============================================================================
// How to install:
 ...
 * If you want to see how it's used please look at the code or look at
 * QuasiNameInput.js and QuasiEzJSON.js as they both use or create
 * Window_TextInput
 * =============================================================================
 * Links
 *  - http://quasixi.com/mv/input/
...
var QuasiInput = (function() {
  ...
  QuasiInput.remap = function(key) {
    switch(key) {
      case "tab":
        return this.public.remapped["tab"];
        break;
      case "ok":
        return this.public.remapped["ok"];
        break;
      case "shift":
        return this.public.remapped["shift"];
        break;
      case "control":
        return this.public.remapped["control"];
        break;
      case "escape":
      case "cancel":
        return this.public.remapped["escape"];
        break;
      case "pageup":
        return this.public.remapped["pageup"];
        break;
      case "pagedown":
        return this.public.remapped["pagedown"];
        break;
      case "left":
        return this.public.remapped["left"];
        break;
      case "right":
        return this.public.remapped["right"];
        break;
      case "up":
        return this.public.remapped["up"];
        break;
      case "down":
        return this.public.remapped["down"];
        break;
      case "debug":
        return this.public.remapped["debug"];
        break;
      case "fps":
        return Number(this.keyAt(this.public.remapped["fps"]) || 113);
        break;
      case "streched":
        return Number(this.keyAt(this.public.remapped["streched"]) || 114);
        break;
      case "fullscreen":
        return Number(this.keyAt(this.public.remapped["fullscreen"]) || 115);
        break;
      case "restart":
        return Number(this.keyAt(this.public.remapped["restart"]) || 116);
        break;
      case "console":
        return Number(this.keyAt(this.public.remapped["console"]) || 119);
        break;
    }
  };

  QuasiInput.public.sym = {
    "1": "!", "2": "@", "3": "#", "4": "$", "5": "%", "6": "^", "7": "&",
    "8": "*", "9": "(", "0": ")",
    "semicolon": ";",  "equal": "=", "comma": ",", "minus": "-", "period": ".",
    "slash": "/", "grave": "`", "openbracket": "[", "backslash": "\\",
    "closedbracket": "]", "singlequote": "'"
  };

  QuasiInput.public.symUppercase = {
    "semicolon": ":",  "equal": "+", "comma": "<", "minus": "_", "period": ">",
    "slash": "?", "grave": "~", "openbracket": "{", "backslash": "|",
    "closedbracket": "}", "singlequote": "\""
  };
  ...
  Input._setupEventHandlers = function() {
    ...
    document.addEventListener('keypress', this._setCapsLock.bind(this));
...
Window_TextInput.prototype.update = function() {
  ...
  if (this.active && this.visible) {
    if (this._mode === "~a-z0-9") {
      this.checkLetters();
      this.checkNumbers();
      this.checkSymbols();
    } else if (this._mode === "a-z0-9") {
      this.checkLetters();
      this.checkNumbers();
    } else if (this._mode === "a-z") {
      this.checkLetters();
    } else if (this._mode === "0-9") {
      this.checkNumbers();
    }
    ...
    if (Input.isTriggered("#esc")) {
      this.callHandler("#esc");
    }
  }
};

Window_TextInput.prototype.checkLetters = function() {
  if (Input.anyTriggered("a-z")) {
    if (Input.isPressed("#shift") || Input.capsLock) {
      this.add(Input._lastTriggered.toUpperCase());
    } else {
      this.add(Input._lastTriggered);
    }
  }
...
Window_TextInput.prototype.checkNumbers = function() {
  if (Input.anyTriggered("0-9")) {
    if (/^num/.test(Input._lastTriggered)) {
      this.add(Input._lastTriggered[3]);
    } else {
      if (/^~/.test(this._mode) && Input.isPressed("#shift")) {
        this.add(QuasiInput.sym[Input._lastTriggered]);
      } else {
        this.add(Input._lastTriggered);
      }
    }
...
Window_TextInput.prototype.checkSymbols = function() {
  if (Input.anyTriggered("sym")) {
    if (Input.isPressed("#shift")) {
      this.add(QuasiInput.symUppercase[Input._lastTriggered]);
    } else {
      this.add(QuasiInput.sym[Input._lastTriggered]);
    }
  }
